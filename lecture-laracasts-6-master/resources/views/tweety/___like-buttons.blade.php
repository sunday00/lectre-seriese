<form action="{{ route("tweety.prefer.like", $tweet->id) }}" method="POST">
    @csrf
    @method('PATCH')
    <button class="flex items-center mr-4 {{ $tweet->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }} ">
        <svg viewBox="0 0 20 20" class="mr-1 my-2 w-4">
        ...
        <span class="text-s">{{ $tweet->likes }}</span>
    </button>
</form>
<form action="{{ route('tweety.prefer.dislike', $tweet->id) }}" method="POST">
    @csrf
    @method('DELETE')
    <button class="flex items-center {{ $tweet->isDislikedBy(current_user()) ? 'text-red-500' : 'text-gray-500' }}">
        <svg viewBox="0 0 20 20" class="mr-1 my-2 w-4">
            <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                <g class="fill-current">
        ...
        <span class="text-s">{{ $tweet->dislikes }}</span>
    </button>
</form>